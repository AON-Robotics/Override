...
converts inches/handedness/final heading to native AON poses. Speeds are stored as rounded bytes; editor
metadata is not emitted. The robot anchors these poses to its live starting
...
def render(routes, frames):
    ...
    for name, points in sorted(routes.items()):
        ...
        values = ",\n".join("      {" + ", ".join(f"{v:.9f}f" for v in row[:2]) + "}" for row in points)
        speeds = ", ".join(str(row[3]) for row in points)
        branches.append("  if (name == " + json.dumps(name) + ") {\n"
                        "    static constexpr float xy[][2] = {\n" + values + "\n    };\n"
...
inline PathRoute staticRouteAt(const Pose& start, std::string_view name = "path") {
  PathRoute route;
''' + " else\n".join(branches) + '''
...
inline std::vector<Pose> staticPathAt(const Pose& start, std::string_view name = "path") {
  return staticRouteAt(start, name).points;
}
}  // namespace aon::generated
'''
